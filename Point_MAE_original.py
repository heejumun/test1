...
class MaskTransformer(nn.Module):
    ...
    def forward(self, neighborhood, center, noaug = False):
            ...
            pos = self.pos_embed(masked_center) #transformer block 돌리기 위한 positional embedding
...
class Point_MAE(nn.Module):
    ...
    def forward(self, pts, vis = False, **kwargs):
        ...
        x_vis, mask = self.MAE_encoder(neighborhood, center) #encoder positional embedding 
        #print(x_vis.shape)
        ...
        pos_full = torch.cat([pos_emd_vis, pos_emd_mask], dim=1) #decoder 돌리기 위한 positional embedding
        x_rec = self.MAE_decoder(x_full, pos_full, N)         
        B, M, C = x_rec.shape            
        rebuild_points = self.increase_dim(x_rec.transpose(1, 2)).transpose(1, 2).reshape(B * M, -1, 3)  # B M 1024
        gt_points = neighborhood[mask].reshape(B*M,-1,3)
        rebuild_points = rebuild_points[~torch.isnan(rebuild_points)].reshape(B*M,-1,3)
        gt_points = gt_points[~torch.isnan(gt_points)].reshape(B*M,-1,3)
        ...
        if vis: #visualization
            vis_points = neighborhood[~mask].reshape(-1,self.group_size, 3) # (B*(group_num-M), -1,3)
            print(vis_points.shape)
            print(center[~mask].shape)
